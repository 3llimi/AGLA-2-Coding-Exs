...
bool diagonallyDominant(const SquareMatrix& A) {
    int n = A.m;
    for (int i = 0; i < n; i++) {
        double diag = fabs(A.v[i][i]);
        double sum = 0;
        for (int j = 0; j < n; j++) {
            if (i != j) {
                sum += fabs(A.v[i][j]);
...
double calculateAccuracy(const ColumnVector& Xn, const ColumnVector& Xp) {
    double acc = 0;
    for (int i = 0; i < Xn.n; i++) {
        acc += pow(Xn.v[i] - Xp.v[i], 2);
    }
    return sqrt(acc);
}

void SeidelMethod(int n, SquareMatrix* SM, ColumnVector& Bet, double acc) {
    Matrix* A = new SquareMatrix(n);
    *A = *SM;

    if (!diagonallyDominant(*(SquareMatrix*)A)) {
        cout << "The method is not applicable!\n";
        return;
    }

    Matrix* I = new IdentityMatrix(n);
    Matrix* D = new SquareMatrix(n);
    for (int i = 0; i < n; i++) {
        D->v[i][i] = A->v[i][i];
    }

    Matrix* Alpha = new SquareMatrix(n);
    inverse((SquareMatrix*)D);
    *Alpha = *I - (*D * *A);

    ColumnVector Beta(n);
    Beta = *D * Bet;
    Matrix* B = new SquareMatrix(n);
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            if(j<=i){
                B->v[i][j] = Alpha->v[i][j];
    ...
    Matrix* IB = new SquareMatrix(n);
    *IB = *I - *B;
    Matrix* invIB = new SquareMatrix(n);
    *invIB = *IB;
    inverse((SquareMatrix *)invIB);
    cout << "alpha:\n";
    Alpha->outputWithout();
    cout << "beta:\n";
    for (int i = 0; i < n; i++) {
        cout << fixed << setprecision(4) << Beta.v[i] << endl;
    }
    cout << "B:\n";
    B->outputWithout();
    cout << "C:\n";
    ...
    while (true) {
        ...
        if (flag == false) {
            cout << "x(" << steps << "):\n";
            for (int i = 0; i < n; i++) {
                cout << fixed << setprecision(4) << Xp.v[i] << endl;
            }
            cout << "e: " << currentAcc << endl;
            cout << "x~:\n";
            for (int i = 0; i < n; i++) {
                cout << fixed << setprecision(4) << Xp.v[i] << endl;
            }
        ...
        currentAcc = calculateAccuracy(Xn, Xp);
        if (currentAcc <= acc) flag = false;
        steps++;
        Xp = Xn;
    }
}
...
int main() {
    ...
    SeidelMethod(dimension, &A, b, epsilon);
